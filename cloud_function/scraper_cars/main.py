...
# Per-listing scraper: fetches search results and saves visible text from each listing page.
...
# ---- Config (set via GitHub Repository Variables → GCP --set-env-vars) ----
BUCKET_NAME        = os.environ["BUCKET_NAME"]
BASE_SITE          = os.environ["BASE_SITE"]
SEARCH_PATH        = os.environ.get("SEARCH_PATH", "/search/cto")
...
def _page_url(base: str, path: str, page: int) -> str:
    # search pagination: s=<offset>, 120 results/page
    if page == 0:
...
def entrypoint(request: Request):
    """HTTP GET. Optional query overrides: ?pages=2&max=40&base=<BASE_SITE>&path=<SEARCH_PATH>"""
